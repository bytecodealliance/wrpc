...
/// ### Imports: Resources
///
/// Imported resources generate a marker type named after the resource. This
/// type is not constructed directly; instead it parameterizes the
/// `ResourceOwn<T>` and `ResourceBorrow<T>` handle types from `wrpc_transport`,
/// which represent owned and borrowed handles respectively. The resource's
/// constructor, methods, and static functions are generated as associated
/// functions on the marker type, each taking the wRPC transport and a context
/// as their first two arguments; methods additionally take a
/// `&ResourceBorrow<T>` receiver, obtained from an owned handle via
/// `ResourceOwn::as_borrow`.
///
/// ```
/// use wit_bindgen_wrpc::generate;
///
...
///     Logger::log(wrpc, (), &logger.as_borrow(), Level::Debug, "This is a global message").await?;
///
...
///
/// Additionally a function is generated by `generate!` called `serve`. The
/// `serve` function is given a value that implements the export `trait`s and
/// will serve all of the world's exported functions over the provided wRPC
/// transport.
///
...
/// the server owns the resource's lifecycle, creating new handles of that type
/// at any time and resolving them back to its own backing storage when methods
/// are invoked.
///
/// As with imports, an exported resource generates a marker type named after
/// the resource, and owned/borrowed handles are expressed as `ResourceOwn<T>`
/// and `ResourceBorrow<T>` from `wrpc_transport`. Unlike imports, the server
/// mints and dereferences these handles itself: the handle payload is an opaque
/// `bytes::Bytes` blob whose meaning is entirely up to the `Handler`
/// implementation (in the example below it indexes into a `Vec`).
///
/// Like `interface`s, the methods, constructor, and static functions of a
/// `resource` are packaged up into a `trait` named `Handler` followed by the
/// resource name (e.g. `HandlerLogger`), which must be implemented for your
/// type. The `Handler` trait of the resource's containing interface/world has
/// this per-resource trait as a supertrait, so it must be implemented as well —
/// even when the interface itself has no functions of its own:
///
/// ```
...
/// }
///
/// // The `logging` interface has no functions of its own, but its `Handler`
/// // trait requires the per-resource `HandlerLogger` trait as a supertrait, so
/// // it must still be implemented here — with an empty body.
/// impl<Ctx: Send> Handler<Ctx> for MyComponent {}
///
/// struct MyLogger {
///     level: RwLock<Level>,
...
///
/// Exported functions are served at runtime by the generated `serve` function,
/// which is not created when `generate!` is invoked. Instead you first define
/// your own type and implement the various `Handler` `trait`s for it, and then
/// pass it to `serve` along with a `wrpc_transport::Serve` transport to start
/// handling invocations.
///
/// Each call to `generate!` itself generates a `serve` function adjacent to the
/// generated bindings. Its first argument is the transport to serve on and its
/// second argument is a value that implements the generated `Handler` traits:
///
/// ```
/// use futures::stream::TryStreamExt as _;
/// use wit_bindgen_wrpc::generate;
...
/// ```
///
/// The value passed to `serve` is a Rust type which implements the `Handler`
/// traits generated by `generate!`. Note that all `Handler` traits must be
/// implemented for the type provided or an error will be generated.
///
/// If the `generate!` macro was run in a different module then `serve` is
/// namespaced under that module, just like the rest of the generated bindings:
///
/// ```
/// use futures::stream::TryStreamExt as _;
///
...
///
///     // Names of `record`s and `variant`s, as written in the WIT (i.e. in
///     // kebab-case), to exclude when applying `additional_derives`. This is
///     // useful when one of the requested derives cannot be implemented for a
///     // particular generated type.
///     //
///     // By default this set is empty.
///     additional_derives_ignore: ["foo", "bar"],
///
...
///     // wit-bindgen-wrpc generate (including alignment, etc.), since
...
///
///     // Configure where the runtime dependencies of the generated code are
///     // located. By default each of these points at the corresponding crate
///     // reexported from `wit-bindgen-wrpc` (e.g. `wit_bindgen_wrpc::anyhow`),
///     // assuming that your crate depends on the `wit-bindgen-wrpc` crate
///     // itself.
///     anyhow_path: "path::to::anyhow",
///     bytes_path: "path::to::bytes",
///     futures_path: "path::to::futures",
///     tokio_path: "path::to::tokio",
///     tokio_util_path: "path::to::tokio_util",
///     tracing_path: "path::to::tracing",
///     wasm_tokio_path: "path::to::wasm_tokio",
///     wrpc_transport_path: "path::to::wrpc_transport",
///
...
///     // is primarily useful for `wit-bindgen-wrpc` developers.
///     //
...
/// Compile-fail tests mirroring upstream `wit-bindgen`'s
/// `crates/rust/tests/codegen.rs` modules for features the wRPC generator does
/// **not** implement yet. They are kept as `compile_fail` doctests rather than
/// regular codegen tests (which would break the build) to track parity with
...
/// ```
///
/// `map` types are not supported even with the default map type:
///
/// ```compile_fail
/// wit_bindgen_wrpc::generate!({
///     inline: r#"
...
/// ```
///
/// The `enable_method_chaining` option is not supported (upstream #1586):
///
/// ```compile_fail
/// wit_bindgen_wrpc::generate!({
///     inline: r#"
...
/// ```
#[cfg(doctest)]
pub struct UnimplementedFeatureTests;
