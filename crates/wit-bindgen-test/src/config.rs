...
impl From<StringList> for Vec<String> {
    fn from(list: StringList) -> Vec<String> {
        match list {
            StringList::String(s) => s.split_whitespace().map(ToString::to_string).collect(),
            StringList::List(s) => s,
        }
    }
}
...
///
/// Unknown fields are tolerated (unlike upstream): the `//@` markers on the
/// vendored `tests/codegen/*.wit` files gain new keys (`error-context`,
/// `stream`, ...) across wit-bindgen versions, and wRPC drives expected
/// failures by test name in `should_fail_verify` rather than from this config,
/// so accepting-and-ignoring unknown keys keeps this crate version-stable.
...
pub struct WitConfig {
    ...
    ///
    /// Currently accepted (so `//@ async = true` markers carried from upstream
    /// parse) but not consulted: the wRPC generators' actual async support is
    /// expressed per-test in `should_fail_verify` instead.
    #[serde(default, rename = "async")]
    #[allow(dead_code)]
    pub async_: bool,

    /// When set to `true` disables the passing of per-language default bindgen
    /// arguments. For example with Rust it avoids passing `--generate_all` by
