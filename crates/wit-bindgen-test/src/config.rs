...
pub struct RuntimeTestConfig<T = HashMap<String, toml::Value>> {
    ...
    #[expect(
        dead_code,
        reason = "wasmtime runner flags; the wRPC host runtime driver has no wasmtime"
    )]
    ...
    #[expect(
        dead_code,
        reason = "no per-language config consumed by the wRPC runtime driver"
    )]
    pub lang: Option<T>,
}
...
#[expect(
    dead_code,
    reason = "fields are carried from upstream; wRPC drives expected failures by test name in should_fail_verify"
)]
