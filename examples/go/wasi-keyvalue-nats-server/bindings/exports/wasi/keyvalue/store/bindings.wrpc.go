...
func ServeInterface(s wrpc.Server, h Handler) (stop func() error, err error) {
	...
	stop0, err := s.Serve("wasi:keyvalue/store@0.2.0-draft", "open", func(ctx context.Context, w wrpc.IndexWriteCloser, r wrpc.IndexReadCloser) {
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result value", "i", 0, "wasi:keyvalue/store@0.2.0-draft", "name", "open", "err", err)
			return
		}
		if write0 != nil {
			writes[0] = write0
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result", "wasi:keyvalue/store@0.2.0-draft", "name", "open", "err", err)
			return
		}
		if len(writes) > 0 {
			for index, write := range writes {
				w, err := w.Index(index)
				if err != nil {
					slog.ErrorContext(ctx, "failed to index writer", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "open", "err", err)
					return
				}
				index := index
				write := write
				go func() {
					if err := write(w); err != nil {
						slog.WarnContext(ctx, "failed to write nested result value", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "open", "err", err)
					}
				}()
			}
		}
	...
	stop1, err := s.Serve("wasi:keyvalue/store@0.2.0-draft", "bucket.get", func(ctx context.Context, w wrpc.IndexWriteCloser, r wrpc.IndexReadCloser) {
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result value", "i", 0, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.get", "err", err)
			return
		}
		if write0 != nil {
			writes[0] = write0
		...
		_, err = w.Write(buf.Bytes())
		if err != nil {
			slog.WarnContext(ctx, "failed to write result", "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.get", "err", err)
			return
		}
		if len(writes) > 0 {
			for index, write := range writes {
				w, err := w.Index(index)
				if err != nil {
					slog.ErrorContext(ctx, "failed to index writer", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.get", "err", err)
					return
				}
				index := index
				write := write
				go func() {
					if err := write(w); err != nil {
						slog.WarnContext(ctx, "failed to write nested result value", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.get", "err", err)
					}
				}()
			}
		}
	...
	stop2, err := s.Serve("wasi:keyvalue/store@0.2.0-draft", "bucket.set", func(ctx context.Context, w wrpc.IndexWriteCloser, r wrpc.IndexReadCloser) {
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result value", "i", 0, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.set", "err", err)
			return
		}
		if write0 != nil {
			writes[0] = write0
		...
		_, err = w.Write(buf.Bytes())
		if err != nil {
			slog.WarnContext(ctx, "failed to write result", "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.set", "err", err)
			return
		}
		if len(writes) > 0 {
			for index, write := range writes {
				w, err := w.Index(index)
				if err != nil {
					slog.ErrorContext(ctx, "failed to index writer", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.set", "err", err)
					return
				}
				index := index
				write := write
				go func() {
					if err := write(w); err != nil {
						slog.WarnContext(ctx, "failed to write nested result value", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.set", "err", err)
					}
				}()
			}
		}
	...
	stop3, err := s.Serve("wasi:keyvalue/store@0.2.0-draft", "bucket.delete", func(ctx context.Context, w wrpc.IndexWriteCloser, r wrpc.IndexReadCloser) {
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result value", "i", 0, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.delete", "err", err)
			return
		}
		if write0 != nil {
			writes[0] = write0
		...
		_, err = w.Write(buf.Bytes())
		if err != nil {
			slog.WarnContext(ctx, "failed to write result", "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.delete", "err", err)
			return
		}
		if len(writes) > 0 {
			for index, write := range writes {
				w, err := w.Index(index)
				if err != nil {
					slog.ErrorContext(ctx, "failed to index writer", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.delete", "err", err)
					return
				}
				index := index
				write := write
				go func() {
					if err := write(w); err != nil {
						slog.WarnContext(ctx, "failed to write nested result value", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.delete", "err", err)
					}
				}()
			}
		}
	...
	stop4, err := s.Serve("wasi:keyvalue/store@0.2.0-draft", "bucket.exists", func(ctx context.Context, w wrpc.IndexWriteCloser, r wrpc.IndexReadCloser) {
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result value", "i", 0, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.exists", "err", err)
			return
		}
		if write0 != nil {
			writes[0] = write0
		...
		_, err = w.Write(buf.Bytes())
		if err != nil {
			slog.WarnContext(ctx, "failed to write result", "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.exists", "err", err)
			return
		}
		if len(writes) > 0 {
			for index, write := range writes {
				w, err := w.Index(index)
				if err != nil {
					slog.ErrorContext(ctx, "failed to index writer", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.exists", "err", err)
					return
				}
				index := index
				write := write
				go func() {
					if err := write(w); err != nil {
						slog.WarnContext(ctx, "failed to write nested result value", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.exists", "err", err)
					}
				}()
			}
		}
	...
	stop5, err := s.Serve("wasi:keyvalue/store@0.2.0-draft", "bucket.list-keys", func(ctx context.Context, w wrpc.IndexWriteCloser, r wrpc.IndexReadCloser) {
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result value", "i", 0, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.list-keys", "err", err)
			return
		}
		if write0 != nil {
			writes[0] = write0
		...
		_, err = w.Write(buf.Bytes())
		if err != nil {
			slog.WarnContext(ctx, "failed to write result", "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.list-keys", "err", err)
			return
		}
		if len(writes) > 0 {
			for index, write := range writes {
				w, err := w.Index(index)
				if err != nil {
					slog.ErrorContext(ctx, "failed to index writer", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.list-keys", "err", err)
					return
				}
				index := index
				write := write
				go func() {
					if err := write(w); err != nil {
						slog.WarnContext(ctx, "failed to write nested result value", "index", index, "wasi:keyvalue/store@0.2.0-draft", "name", "bucket.list-keys", "err", err)
					}
				}()
			}
		}
