...
func main() {
	// Handle links, health checks, and shutdown requests
	providerErr := make(chan error)
	wrpcPrefix := make(chan string)
	go func() {
		p, err := provider.New()
		if err != nil {
			providerErr <- err
		...
		wrpcPrefix <- fmt.Sprintf("%s.%s", p.HostData().LatticeRPCPrefix, p.HostData().ProviderKey)

		err = p.Start()
		providerErr <- err
	}()

	// Handle incoming RPC requests, generated by wRPC
	wrpcErr := make(chan error)
	go func() {
		if err := run(<-wrpcPrefix); err != nil {
			wrpcErr <- err
		}
	}()

	// Wait for either of the goroutines to exit
	select {
